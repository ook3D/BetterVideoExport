 ...
 * https://github.com/citizenfx/fivem/blob/master/code/client/shared/Hooking.Patterns.h
		...
		inline pattern&& count(uint32_t expected)
		{
			...
			if (m_matches.size() != expected)
			{
				...
				_snprintf_s(message, _TRUNCATE,
					"Pattern scan failed.\n\nPattern: %s\nExpected matches: %u\nFound: %zu",
					m_patternString.c_str(), expected, m_matches.size());
				MessageBoxA(nullptr, message, "ASI: pattern not found", MB_OK | MB_ICONERROR);
				throw std::runtime_error("hook::pattern: pattern not found or match count mismatch");
			}
